...
<body class="h-full bg-slate-100 font-sans text-sm text-slate-800 antialiased">
    <div class="min-h-full">
        <a href="{{ route('home') }}" class="absolute left-4 top-4 z-10 inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-[13px] font-medium text-slate-600 transition hover:bg-white hover:text-brand-700">
            ← Rudi Nyumbani
        </a>
        ...
        <div class="mx-auto flex min-h-full max-w-5xl flex-col justify-center px-4 py-16 lg:flex-row lg:items-stretch lg:gap-0 lg:py-12">
            ...
            <div class="w-full rounded-2xl border border-slate-200/80 bg-white p-8 shadow-sm lg:max-w-none lg:flex-1 lg:rounded-l-none lg:border-l-0 lg:p-10">
                ...
                <form method="post" action="{{ route('login.post') }}" class="space-y-5">
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-3 text-[13px]">
                        <label class="inline-flex cursor-pointer items-center gap-2 text-slate-600">
                            <input type="checkbox" name="remember" value="1" class="h-4 w-4 rounded border-slate-300 text-brand-600 focus:ring-brand-500" {{ old('remember') ? 'checked' : '' }}>
                            Kumbuka mimi
                        </label>
                        <a href="{{ route('password.otp.request') }}" class="font-medium text-brand-700 hover:text-brand-800">Umesahau neno siri?</a>
                    </div>
                ...
                <p class="mt-8 border-t border-slate-100 pt-6 text-center text-[13px] text-slate-600">
                    Huna akaunti?
                    <a href="{{ route('register') }}" class="font-semibold text-brand-700 hover:text-brand-800">Jisajili</a>
                </p>
            </div>
        </div>
    </div>
