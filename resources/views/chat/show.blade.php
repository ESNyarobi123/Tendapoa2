@extends('layouts.app')
@section('title', 'Mazungumzo — ' . Str::limit($job->title, 40))
  ...
  <main class="tp-main flex min-h-0 w-full min-w-0 flex-1 flex-col overflow-hidden pt-16 lg:pt-6">
    <div class="flex min-h-0 flex-1 flex-col px-4 pb-4 sm:px-6">
      <div class="mx-auto flex min-h-0 w-full max-w-6xl flex-1 flex-col">
        ...
        @if(session('success'))
          <div class="mb-3 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-[13px] font-semibold text-emerald-900 shadow-sm">{{ session('success') }}</div>
        @endif
        @if(session('error'))
          <div class="mb-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-[13px] font-semibold text-red-800 shadow-sm">{{ session('error') }}</div>
        @endif
        ...
        <header class="flex shrink-0 items-center gap-3 rounded-t-2xl border border-b-0 border-slate-200/80 bg-white px-3 py-3 shadow-sm sm:gap-4 sm:px-4 sm:py-4">
          <a href="{{ route('chat.index') }}" class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-slate-200 bg-slate-50 text-slate-600 transition hover:border-brand-200 hover:bg-brand-50 hover:text-brand-700" title="Rudi orodha">
          ...
          <div class="flex min-w-0 flex-1 items-center gap-3">
            ...
            <div class="min-w-0">
              <h1 class="truncate text-[15px] font-bold text-slate-900 sm:text-base">{{ $otherUser->name }}</h1>
              <p class="mt-0.5 flex flex-wrap items-center gap-2 text-[11px] text-slate-500">
                <span class="rounded-md px-2 py-0.5 font-semibold {{ $otherUser->role === 'mfanyakazi' ? 'bg-emerald-100 text-emerald-800' : 'bg-indigo-100 text-indigo-800' }}">
                  {{ $otherUser->role === 'mfanyakazi' ? 'Mfanyakazi' : 'Muhitaji' }}
                </span>
                <span class="truncate">{{ $job->category?->name ?? '' }}</span>
                @if($workerThread)
          ...
          <a href="{{ route('jobs.show', $job) }}" class="hidden shrink-0 rounded-xl bg-gradient-to-r from-brand-600 to-indigo-600 px-3 py-2 text-[12px] font-bold text-white shadow-md transition hover:from-brand-700 hover:to-indigo-700 sm:inline-flex sm:items-center sm:gap-1.5">
            <span>📋</span> Kazi
          </a>
        </header>
        ...
        @if($workerThread)
          <div class="flex shrink-0 items-center gap-2 border-x border-violet-200/80 bg-gradient-to-r from-violet-50 to-indigo-50/40 px-3 py-2 text-[11px] text-violet-950 sm:px-4">
            ...
            <span>Mazungumzo <strong>mahususi</strong> na <strong>{{ $otherUser->name }}</strong> pekee kwenye kazi hii — kila mfanyakazi ana mstari wake.</span>
          </div>
        @endif

        {{-- Job strip --}}
        <div class="flex shrink-0 flex-wrap items-center justify-between gap-2 border-x border-slate-200/80 bg-gradient-to-r px-3 py-2.5 sm:px-4 {{ $bannerClass }}">
          <div class="min-w-0">
            <p class="truncate text-[12px] font-bold text-slate-800 sm:text-[13px]">📋 {{ $job->title }}</p>
            <p class="text-[11px] font-semibold tabular-nums text-slate-600">{{ number_format($job->price) }} TZS</p>
          </div>
          <span class="shrink-0 rounded-lg px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide ring-1 ring-slate-200/80 {{ match ($st) {
          ...
          <div class="messages-inner space-y-4">
            @if($messages->isEmpty())
              <div class="empty-state rounded-2xl border border-dashed border-slate-200 bg-white px-6 py-12 text-center">
                ...
                <p class="mx-auto mt-1 max-w-sm text-[12px] text-slate-600">Tuma ujumbe wa kwanza kwa {{ $otherUser->name }}.</p>
              </div>
            @else
              @foreach($messages as $message)
                ...
                @else
                  <div class="message-wrapper flex {{ $message->sender_id === auth()->id() ? 'sent justify-end' : 'received justify-start' }}">
                    <div class="message-content max-w-[min(85%,28rem)]">
                      <div class="message-bubble-wrapper flex gap-2 {{ $message->sender_id === auth()->id() ? 'flex-row-reverse' : 'flex-row' }}">
                        ...
                        <div class="min-w-0">
                          <div class="message-bubble rounded-2xl px-3.5 py-2.5 shadow-sm {{ $message->sender_id === auth()->id() ? 'rounded-br-md bg-gradient-to-br from-brand-600 to-brand-700 text-white' : 'rounded-bl-md border border-slate-200 bg-white text-slate-800' }}">
                            <p class="message-text whitespace-pre-wrap break-words text-[13px] leading-relaxed">{{ $message->message }}</p>
                          </div>
                          <div class="message-meta mt-1 flex items-center gap-2 text-[10px] font-medium {{ $message->sender_id === auth()->id() ? 'justify-end text-brand-600/90' : 'text-slate-400' }}">
        ...
        <a href="{{ route('jobs.show', $job) }}" class="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white py-2.5 text-[12px] font-bold text-slate-700 shadow-sm sm:hidden">
          📋 Angalia kazi
        </a>
