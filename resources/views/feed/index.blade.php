          ...
          <div class="min-w-0 flex-1">
            ...
            <div class="mt-4 flex flex-wrap gap-3">
              <div class="inline-flex items-center gap-2 rounded-xl border border-slate-100 bg-slate-50 px-3 py-2">
                ...
                <div>
                  <div class="text-[15px] font-bold text-slate-900">{{ $jobs->total() }}</div>
              ...
              <div class="inline-flex items-center gap-2 rounded-xl border border-slate-100 bg-slate-50 px-3 py-2">
                <span class="text-lg leading-none">📍</span>
                <div>
                  <div class="text-[15px] font-bold text-brand-700">{{ $jobs->where('distance_info.category', 'near')->count() }}</div>
        ...
        @if(!auth()->user()->hasLocation())
          <div class="mt-5 flex gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-[13px] text-amber-950">
            ...
            <div>
              ...
              <p class="mt-0.5 text-amber-900/80">Weka eneo kwenye wasifu ili kuona umbali. <a href="{{ route('profile.edit') }}" class="font-semibold text-amber-900 underline">Hariri wasifu</a></p>
            </div>
          </div>
        @endif
      </div>
            ...
            @foreach($jobs as $job)
              ...
              <article class="flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:border-brand-200 hover:shadow-md">
                <div class="relative h-44 bg-slate-100">
                  <div class="absolute left-0 right-0 top-0 z-[1] h-1 {{ $strip }}"></div>
                  @if($imageUrl)
                  ...
                  <div class="absolute inset-x-0 bottom-0 flex items-end justify-between gap-2 bg-gradient-to-t from-black/70 to-transparent px-3 pb-3 pt-10">
                    <span class="rounded-lg bg-brand-600 px-2.5 py-1 text-[13px] font-bold text-white shadow-sm">{{ number_format($job->price) }} <span class="text-[10px] font-semibold opacity-90">TZS</span></span>
                    <span class="rounded-lg bg-white/95 px-2 py-1 text-[11px] font-bold text-slate-800 shadow-sm backdrop-blur-sm">
                      @if($job->distance_info['distance'] ?? null)
                        {{ $job->distance_info['distance'] }} km
                      @else
                        {{ $job->distance_info['label'] ?? '—' }}
                      @endif
                    </span>
                  </div>
                </div>
                <div class="flex flex-1 flex-col p-4">
                  <div class="mb-2 flex flex-wrap gap-1.5">
                    <span class="rounded-md bg-brand-50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-brand-800">{{ $job->category->name }}</span>
                  ...
                  <h2 class="line-clamp-2 text-[15px] font-bold leading-snug text-slate-900">{{ $job->title }}</h2>
                  <p class="mt-2 line-clamp-2 flex-1 text-[12px] leading-relaxed text-slate-500">
                    {{ $job->description ? Str::limit(strip_tags($job->description), 100) : 'Hakuna maelezo.' }}
                  </p>
                  <div class="mt-3 flex items-center justify-between gap-2 border-t border-slate-100 pt-3 text-[11px] text-slate-400">
                    <span>{{ $job->created_at->diffForHumans() }}</span>
                    <a href="{{ route('jobs.show', $job) }}" class="inline-flex items-center gap-1 rounded-lg bg-brand-600 px-3 py-1.5 text-[12px] font-semibold text-white transition hover:bg-brand-700">Angalia</a>
                  </div>
                </div>
              </article>
            @endforeach
...
<div id="feed-data" data-user-lat="{{ $userLat ?? '' }}" data-user-lng="{{ $userLng ?? '' }}"
  data-jobs="{{ json_encode($jobsArray) }}" data-all-jobs="{{ json_encode($allJobsFromDb) }}" class="hidden"></div>
