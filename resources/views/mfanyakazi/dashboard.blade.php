    ...
    <div class="mx-auto max-w-6xl space-y-6">
      ...
      <section class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-teal-700 via-brand-600 to-emerald-600 p-6 text-white shadow-lg sm:p-8">
        ...
        <div class="relative">
          ...
          <h1 class="mt-2 text-2xl font-bold tracking-tight sm:text-3xl">Karibu, {{ Auth::user()->name ?? 'Mfanyakazi' }}</h1>
          ...
          <div class="mt-5 flex flex-wrap gap-2">
            <a href="{{ $feedUrl }}" class="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-[13px] font-bold text-brand-800 shadow-md transition hover:bg-brand-50">
            ...
            <a href="{{ $assignedUrl }}" class="inline-flex items-center gap-2 rounded-xl border border-white/30 bg-white/10 px-4 py-2.5 text-[13px] font-semibold text-white backdrop-blur-sm transition hover:bg-white/20">Kazi zangu</a>
      ...
      <x-dashboard-attention-jobs :jobs="$attentionJobs ?? collect()" role="mfanyakazi" />
      ...
      <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <article class="relative overflow-hidden rounded-2xl border border-emerald-100 bg-gradient-to-br from-emerald-50 to-white p-5 shadow-sm ring-1 ring-emerald-100/80">
          ...
          <p class="mt-1 text-3xl font-bold tabular-nums text-slate-900">{{ number_format($done) }}</p>
        ...
        <article class="relative overflow-hidden rounded-2xl border border-violet-100 bg-gradient-to-br from-violet-50 to-white p-5 shadow-sm ring-1 ring-violet-100/80">
          ...
          <p class="mt-1 text-xl font-bold tabular-nums text-violet-900">{{ number_format($earned) }} <span class="text-[11px] font-semibold text-violet-600">TZS</span></p>
          <p class="mt-1 text-[11px] text-slate-500">Mwezi huu: {{ number_format($thisMonthEarnings) }} TZS</p>
        </article>
        ...
        <article class="relative overflow-hidden rounded-2xl border border-amber-100 bg-gradient-to-br from-amber-50 to-white p-5 shadow-sm ring-1 ring-amber-100/80">
          ...
          <p class="mt-1 text-xl font-bold tabular-nums text-slate-900">{{ number_format($available) }} <span class="text-[11px] font-semibold text-slate-500">TZS</span></p>
          <a href="{{ $withdrawUrl }}" class="mt-1 inline-flex text-[11px] font-bold text-amber-800 hover:underline">Toa pesa →</a>
        </article>
        ...
        <article class="relative overflow-hidden rounded-2xl border border-teal-200/60 bg-gradient-to-br from-teal-50 via-brand-50 to-white p-5 shadow-sm ring-1 ring-teal-100/80">
          ...
          <p class="mt-1 text-3xl font-bold tabular-nums text-brand-800">{{ number_format($avgCompletionTime, 1) }} <span class="text-lg font-semibold text-brand-600">siku</span></p>
            ...
            @foreach($notifications->take(5) as $notif)
              <div class="flex gap-3 rounded-xl border border-slate-100 bg-slate-50/80 px-4 py-3 transition hover:bg-white hover:shadow-sm">
                <span class="text-xl leading-none">
                  @if(($notif->data['type'] ?? '') === 'admin_message') 📢
                  @elseif(str_contains($notif->data['type'] ?? '', 'job')) 💼
                  @elseif(str_contains($notif->data['type'] ?? '', 'payment')) 💰
                  @else 🔔 @endif
                </span>
                <div class="min-w-0 flex-1">
                  <p class="text-[13px] font-semibold text-slate-900">{{ $notif->data['title'] ?? 'Taarifa' }}</p>
                  <p class="text-[12px] text-slate-600">{{ $notif->data['message'] ?? '' }}</p>
      ...
      <div class="grid gap-6 lg:grid-cols-2">
        ...
        <div class="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-md">
          <div class="flex items-center justify-between border-b border-slate-100 bg-gradient-to-r from-brand-50/60 to-white px-5 py-4">
            ...
            <a href="{{ $assignedUrl }}" class="text-[12px] font-bold text-brand-700 hover:underline">Ona zote</a>
          </div>
          <div class="p-5">
              ...
              @foreach($currentJobs as $job)
                ...
                <div class="job-card rounded-xl border border-slate-100 bg-slate-50/80 p-4"
                  ...
                  <div class="flex flex-wrap items-start justify-between gap-2">
                    <div class="min-w-0">
                      <p class="font-semibold text-slate-900">{{ $job->title ?? 'Kazi' }}</p>
                      <p class="mt-1 text-[11px] text-slate-500">📍 {{ $job->location ?? '—' }} · {{ $job->created_at?->diffForHumans() ?? '' }}</p>
                    </div>
                    <span class="shrink-0 rounded-full bg-white px-2.5 py-0.5 text-[10px] font-bold uppercase text-slate-600 ring-1 ring-slate-200">{{ ucfirst(str_replace('_', ' ', $job->status ?? '')) }}</span>
                  </div>
                  <div class="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-slate-200/80 pt-3">
                    <span class="text-[14px] font-bold text-brand-700">TZS {{ number_format($amt) }}</span>
                    <div>
                      @if($job->status === 'assigned')
                        <form method="POST" action="{{ route('mfanyakazi.jobs.accept', $job->id) }}" class="inline">
                          @csrf
                      ...
                      @elseif($job->status === 'in_progress')
                        <button type="button" onclick="showCodeInputModal({{ $job->id }}, @json($job->title))" class="rounded-lg bg-slate-900 px-3 py-1.5 text-[12px] font-semibold text-white hover:bg-slate-800">Maliza kazi</button>
          ...
          @else
            <div class="py-10 text-center text-[13px] text-slate-500">
              ...
              <a href="{{ $feedUrl }}" class="mt-3 inline-flex rounded-lg bg-brand-600 px-4 py-2 text-[13px] font-semibold text-white hover:bg-brand-700">Tafuta kazi</a>
            </div>
          @endif
          </div>
        </div>
      ...
      <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <a href="{{ $feedUrl }}" class="group flex items-center justify-center gap-2 rounded-2xl border border-indigo-100 bg-gradient-to-br from-indigo-50 to-white px-4 py-4 text-center text-[13px] font-bold text-indigo-900 shadow-sm ring-1 ring-indigo-100/80 transition hover:shadow-md">
          <span class="text-lg">🔍</span> Tafuta kazi mpya
        </a>
        <a href="{{ route('jobs.create-mfanyakazi') }}" class="group flex items-center justify-center gap-2 rounded-2xl border border-brand-200 bg-gradient-to-br from-brand-50 to-teal-50/50 px-4 py-4 text-center text-[13px] font-bold text-brand-900 shadow-sm ring-1 ring-brand-100 transition hover:shadow-md">
          <span class="text-lg">➕</span> Chapisha huduma
        </a>
        <a href="{{ $withdrawUrl }}" class="flex items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-4 text-center text-[13px] font-bold text-slate-800 shadow-sm transition hover:bg-slate-50 sm:col-span-2 lg:col-span-1 @if($available <= 0) pointer-events-none opacity-50 @endif">
      ...
      <div class="space-y-6">
        <div class="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-md">
          <div class="flex items-center justify-between border-b border-slate-100 bg-gradient-to-r from-emerald-50/80 to-white px-5 py-4">
            ...
            <span class="rounded-full bg-emerald-100 px-2.5 py-0.5 text-[11px] font-bold text-emerald-900">{{ $earningsHistory->count() }}</span>
          </div>
              ...
              @foreach($earningsHistory as $earning)
                <li class="flex items-center justify-between gap-3 px-5 py-3 text-[13px]">
                  <div>
                    ...
                    <p class="text-[11px] text-slate-500">{{ $earning->description ?? '' }} · {{ $earning->created_at->diffForHumans() }}</p>
                  </div>
                  <span class="shrink-0 font-bold text-emerald-700">+{{ number_format($earning->amount) }} TZS</span>
                </li>
              @endforeach
            </ul>
          @else
        ...
        <div class="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-md">
          <div class="flex items-center justify-between border-b border-slate-100 bg-gradient-to-r from-rose-50/60 to-white px-5 py-4">
            ...
            <span class="rounded-full bg-rose-100 px-2.5 py-0.5 text-[11px] font-bold text-rose-900">{{ $withdrawalsHistory->count() }}</span>
          </div>
          @if($withdrawalsHistory->count() > 0)
            <ul class="divide-y divide-slate-100">
              @foreach($withdrawalsHistory as $withdrawal)
                <li class="flex items-center justify-between gap-3 px-5 py-3 text-[13px]">
                  <div>
                    <p class="font-medium text-slate-900">{{ $withdrawal->network_type ? ucfirst($withdrawal->network_type) : 'Malipo' }}</p>
                    <p class="text-[11px] text-slate-500">{{ $withdrawal->account }} · {{ $withdrawal->created_at->diffForHumans() }}</p>
                  </div>
                  <span class="shrink-0 font-bold text-red-600">-{{ number_format($withdrawal->amount) }} TZS</span>
                </li>
              @endforeach
            </ul>
          @else
        ...
        <div class="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-md">
          <div class="flex items-center justify-between border-b border-slate-100 bg-gradient-to-r from-violet-50/70 to-white px-5 py-4">
            ...
            <span class="rounded-full bg-violet-100 px-2.5 py-0.5 text-[11px] font-bold text-violet-900">{{ $completedJobs->count() }}</span>
          </div>
            ...
            <ul class="divide-y divide-slate-100">
              @foreach($completedJobs as $job)
                <li class="flex flex-col gap-1 px-5 py-3 text-[13px] sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <p class="font-medium text-slate-900">{{ $job->title ?? 'Kazi' }}</p>
                    <p class="text-[11px] text-slate-500">{{ $job->muhitaji->name ?? '' }} · {{ $job->category?->name ?? '' }} · {{ $job->completed_at->diffForHumans() }}</p>
                  </div>
                  <span class="font-bold text-emerald-700">+{{ number_format($job->amount ?? $job->price ?? 0) }} TZS</span>
                </li>
              @endforeach
            </ul>
          @else
...
document.getElementById('codeInputForm').addEventListener('submit', function(e) {
  ...
  fetch('{{ url('/mfanyakazi/jobs') }}/' + currentJob.id + '/complete', {
    method: 'POST',
    headers: {
...
document.addEventListener('keydown', function(e) {
  ...
  if (e.key === 'j') { e.preventDefault(); window.location.href = @json($feedUrl); }
  if (e.key === 'a') { e.preventDefault(); window.location.href = @json($assignedUrl); }
  if (e.key === 'w') { e.preventDefault(); window.location.href = @json($withdrawUrl); }
});
</script>
@endsection
