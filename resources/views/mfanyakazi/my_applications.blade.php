    ...
    <div class="mx-auto max-w-6xl space-y-5">
      ...
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex flex-wrap gap-2">
          <a href="{{ route('feed') }}" class="inline-flex items-center justify-center rounded-lg bg-brand-600 px-3 py-2 text-[12px] font-semibold text-white shadow-sm hover:bg-brand-700">Tafuta kazi</a>
          ...
          @foreach($applications as $app)
            ...
            <article class="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between sm:gap-4">
              <div class="min-w-0 flex-1">
                <span class="inline-flex rounded-full px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide ring-1 {{ $stStyle }}">{{ $app->getStatusLabel() }}</span>
                <p class="mt-2 truncate text-[14px] font-bold text-slate-900">{{ $job?->title ?? 'Kazi' }}</p>
                <p class="mt-0.5 text-[12px] text-slate-600">
                  Mteja: <span class="font-semibold text-slate-800">{{ $job?->muhitaji?->name ?? '—' }}</span>
                  @if($job?->category) · {{ $job->category->name }} @endif
                </p>
                <p class="mt-1 text-[12px] font-semibold tabular-nums text-slate-800">
                  Ombi lako: {{ number_format($app->proposed_amount) }} TZS
                ...
                @elseif($app->status === \App\Models\JobApplication::STATUS_REJECTED)
                  <p class="mt-2 text-[11px] text-slate-500">Ombi halijakubaliwa kwenye kazi hii. Unaweza kuomba kazi nyingine.</p>
                @endif
              </div>
              <div class="flex shrink-0 flex-col gap-2 sm:items-end">
                @if($job)
                  <a href="{{ route('jobs.show', $job) }}" class="inline-flex items-center justify-center rounded-lg bg-brand-600 px-4 py-2 text-[12px] font-semibold text-white shadow-sm hover:bg-brand-700">
                    Fungua kazi
                  </a>
                @endif
                <span class="text-[10px] text-slate-400">{{ $app->updated_at->diffForHumans() }}</span>
              </div>
            </article>
          @endforeach
        </div>
      ...
      @else
        <div class="rounded-2xl border border-dashed border-slate-200 bg-white px-6 py-14 text-center">
          ...
          <a href="{{ route('feed') }}" class="mt-5 inline-flex rounded-xl bg-brand-600 px-4 py-2.5 text-[12px] font-bold text-white shadow-sm hover:bg-brand-700">Tafuta kazi</a>
        </div>
      @endif
