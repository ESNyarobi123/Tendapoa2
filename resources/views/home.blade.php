...
<header class="tp-landing-nav" id="landingNav">
    <div class="tp-landing-nav-inner">
        <x-brand-logo href="{{ route('home') }}" class="tp-landing-logo" text-class="text-inherit" size="md" />
        <nav class="tp-landing-nav-links" aria-label="Menyu kuu">
            ...
            @auth
                <a href="{{ route('dashboard') }}" class="tp-landing-btn tp-landing-btn-brand ml-2">Dashibodi</a>
            @else
                <a href="{{ route('login') }}" class="tp-landing-nav-link">Ingia</a>
                <a href="{{ route('register') }}" class="tp-landing-btn tp-landing-btn-primary ml-2">Jisajili bure</a>
            @endauth
        </nav>
    ...
    <div class="tp-landing-mobile-menu" id="mobileMenu">
        ...
        @auth
            <a href="{{ route('dashboard') }}" class="tp-landing-btn tp-landing-btn-brand mt-3 w-full">Dashibodi</a>
        @else
            <a href="{{ route('login') }}" class="tp-landing-nav-link block">Ingia</a>
            <a href="{{ route('register') }}" class="tp-landing-btn tp-landing-btn-primary mt-3 w-full">Jisajili bure</a>
        @endauth
    </div>
</header>

{{-- HERO: maandishi + picha kubwa ya usafi --}}
<section class="tp-landing-hero-split" id="home">
    <div class="tp-landing-hero-copy">
        ...
        <div class="tp-landing-hero-actions">
            <a href="{{ route('register') }}" class="tp-landing-btn tp-landing-btn-primary">
                Anza sasa — bure
            </a>
...
<section class="tp-landing-app-section" id="app">
    <div class="tp-landing-container">
        ...
        <div class="tp-landing-phones">
            @include('partials.landing-phone-client')
            @include('partials.landing-phone-worker')
        </div>
        <div class="mt-12 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <a href="{{ route('register') }}" class="tp-landing-btn tp-landing-btn-primary">
                Jisajili kama Mteja
            </a>
            <a href="{{ $activeVersion ? route('app.download') : '#' }}"
                class="tp-landing-btn tp-landing-btn-secondary {{ $activeVersion ? '' : 'pointer-events-none opacity-50' }}">
                📱 Pakua App{{ $activeVersion ? ' (v'.$activeVersion->version.')' : '' }}
            </a>
        </div>
    </div>
</section>

{{-- Picha za watu wakifanya usafi --}}
    ...
    <div class="tp-landing-container">
        ...
        <div class="tp-landing-cat-grid">
            @foreach($cats->take(8) as $cat)
                <a href="{{ route('register') }}" class="tp-landing-cat">
                    <span class="tp-landing-cat-emoji">{{ $categoryEmoji($cat->name) }}</span>
                    <span class="mt-3 text-[13px] font-bold text-slate-800">{{ $cat->name }}</span>
                </a>
            @endforeach
        </div>
    </div>
    ...
    <div class="tp-landing-container">
        <div class="tp-landing-dual-cta">
            <div class="tp-landing-cta-card tp-landing-cta-client">
                ...
                <a href="{{ route('register') }}" class="tp-landing-btn mt-6 bg-white text-brand-800 hover:bg-slate-100">Anza kama Mteja</a>
            </div>
            <div class="tp-landing-cta-card tp-landing-cta-worker">
                ...
                <a href="{{ route('register') }}" class="tp-landing-btn tp-landing-btn-brand mt-6">Anza kama Mfanyakazi</a>
            </div>
        </div>
    </div>
</section>
...
<footer class="tp-landing-footer">
    <div class="tp-landing-container py-12">
        <div class="grid gap-8 md:grid-cols-3">
            <div>
                <x-brand-logo href="{{ route('home') }}" class="tp-landing-logo tp-landing-logo--light" text-class="text-inherit" size="md" />
                <p class="mt-4 text-[14px] leading-relaxed">Jukwaa la usafi na huduma — salama, rahisi, na la kuaminika.</p>
                <p class="mt-4 text-[12px]">© {{ date('Y') }} TendaPoa</p>
            </div>
            <div>
                ...
                <nav class="mt-3 flex flex-col gap-2 text-[14px]">
                    ...
                    <a href="{{ route('policy.privacy') }}" class="text-slate-400 no-underline hover:text-white">Faragha</a>
                    <a href="{{ route('policy.terms') }}" class="text-slate-400 no-underline hover:text-white">Masharti</a>
                </nav>
            </div>
            <div>
                <a href="{{ route('register') }}" class="tp-landing-btn tp-landing-btn-primary">Jisajili bure</a>
            </div>
        </div>
    </div>
</footer>
