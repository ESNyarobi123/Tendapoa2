<?php $__env->startSection('title', 'Admin — Job Management'); ?>
  ...
  <div class="page-container">
    ...
    <?php if(!empty($filterUser)): ?>
      <div class="adm-filter-banner">
        <span>Kazi za mtumiaji: <strong><?php echo e($filterUser->name); ?></strong> (ID <?php echo e($filterUser->id); ?>)</span>
        <a href="<?php echo e(route('admin.jobs')); ?>">Ondoa kichujio</a>
      </div>
    <?php endif; ?>
    ...
    <div class="stats-grid">
      ...
      <div class="stat-card">
        <div class="stat-icon">📝</div>
        <div class="stat-value"><?php echo e($jobs->where('status', 'posted')->count()); ?></div>
        <div class="stat-label">Posted</div>
      </div>
      <div class="stat-card">
        <div class="stat-icon">⏳</div>
        <div class="stat-value"><?php echo e($jobs->where('status', 'in_progress')->count()); ?></div>
        <div class="stat-label">In Progress</div>
      </div>
      <div class="stat-card">
        <div class="stat-icon">✅</div>
        <div class="stat-value"><?php echo e($jobs->where('status', 'completed')->count()); ?></div>
        <div class="stat-label">Completed</div>
      </div>
      <div class="stat-card">
        <div class="stat-icon">❌</div>
        <div class="stat-value"><?php echo e($jobs->where('status', 'cancelled')->count()); ?></div>
      ...
      <?php if($jobs->count()): ?>
        <div class="jobs-list">
          <?php $__currentLoopData = $jobs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $job): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="job-card" data-status="<?php echo e($job->status); ?>" data-category="<?php echo e($job->category->slug ?? ''); ?>" data-price="<?php echo e($job->price ?? 0); ?>">
              <div class="job-header">
                <div class="job-info">
                  <div class="job-status <?php echo e(str_replace('_', '-', $job->status)); ?>">
                    <?php echo e(strtoupper($job->status)); ?>

                  </div>
                  <h3><?php echo e($job->title); ?></h3>
                  <div class="job-meta">
                    <span>👤 <?php echo e($job->muhitaji->name ?? 'Unknown'); ?></span>
                    <span>🏷️ <?php echo e($job->category->name ?? 'Uncategorized'); ?></span>
                    <span>📍 <?php echo e($job->location ?? 'N/A'); ?></span>
                    <span>⏱️ <?php echo e($job->created_at?->diffForHumans() ?? ''); ?></span>
                  </div>
                </div>
                <div class="job-price">TZS <?php echo e(number_format($job->price ?? 0)); ?></div>
              </div>

              <div class="job-details">
                <div class="detail-row">
                  <span class="detail-label">Description:</span>
                  <span class="detail-value"><?php echo e(Str::limit($job->description ?? 'No description', 50)); ?></span>
                </div>
                <div class="detail-row">
                  <span class="detail-label">Worker:</span>
                  <span class="detail-value"><?php echo e($job->acceptedWorker->name ?? 'Not assigned'); ?></span>
                </div>
                <div class="detail-row">
                  <span class="detail-label">Created:</span>
                  <span class="detail-value"><?php echo e($job->created_at?->format('M d, Y H:i') ?? 'N/A'); ?></span>
                </div>
                <?php if($job->completed_at): ?>
                  <div class="detail-row">
                    <span class="detail-label">Completed:</span>
                    <span class="detail-value"><?php echo e($job->completed_at->format('M d, Y H:i')); ?></span>
                  </div>
                <?php endif; ?>
              </div>

              <div class="job-actions">
                <a class="btn btn-outline" href="<?php echo e(route('jobs.show', $job)); ?>">
                  <span>👁️</span>
                  View Details
                </a>
                <?php if($job->status === 'posted'): ?>
                  <button class="btn btn-warning" onclick="assignJob(<?php echo e($job->id); ?>)">
                    <span>👷</span>
                    Assign Worker
                  </button>
                <?php endif; ?>
                <?php if(in_array($job->status, ['posted', 'assigned'])): ?>
                  <button class="btn btn-danger" onclick="cancelJob(<?php echo e($job->id); ?>)">
                    <span>❌</span>
                    Cancel Job
                  </button>
                <?php endif; ?>
                <?php if($job->status === 'completed'): ?>
                  <button class="btn btn-success" onclick="markAsPaid(<?php echo e($job->id); ?>)">
                    <span>💰</span>
                    Mark as Paid
                  </button>
                <?php endif; ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/eunice/GITHUB COOP/Tendapoa2/resources/views/admin/jobs.blade.php ENDPATH**/ ?>